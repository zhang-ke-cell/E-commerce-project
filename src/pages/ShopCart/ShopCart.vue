  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type,disNum,cart) {
        ...
        switch (type) {
          ...
          case "minus":
            ...
            break
          case "change":
            // 如果用户输进来的最终量时非法的（带有汉字||负数），则带给服务器的数字不变
            if (isNaN(disNum)) {
              disNum = 0
            }
            else {
      ...
      async deleteCartById (cart){
        try{
          await this.$store.dispatch("shopcart/deleteCartBySkuId",cart.skuId)
          this.getData()
        }catch(error){
          alert(error.message)
        }        
