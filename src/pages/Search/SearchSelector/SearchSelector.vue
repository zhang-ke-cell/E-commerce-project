<template>
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="ext">
        <a class="sui-btn" href="javascript:void(0);">多选</a> <a href="javascript:void(0);">更多</a>
      </div>
    </div>
    <!--平台售卖属性的地方-->
...
export default {
  ...
  computed: {...mapGetters('search', ["trademarkList", "attrsList"])},


  methods:{
    // 品牌事件的处理函数
    tradeMarkHandler(trademark) {
      // 点击率品牌（苹果），还是需要整理参数，向服务器发送请求获取相应的数据进行展示
      // 在哪个组件中发送请求，search组件还是Header组件
      // 答：父组件，因为父组件中才有searchParams参数（带给服务器发送请求的参数），子组件把你点击的品牌的信息，需要给父组件传过去
      // 使用自定义的事件实现子给父传参
