...
export default {
  ...
  mounted(){
    //派发actions：通过Vuex发起ajax请求，将数据储存在仓库当中
    this.$store.dispatch('home/getBannerList')

    //在new Swiper实例之前，页面中必须要有html结构（直接把new Swiper实例放在mounted这里发现不行）
    //因为dispatch中涉及到异步语句，导致v-for遍历的时候还没有完全解析成DOM元素（拿不到bannerList数据）
    //即执行顺序为actions new swiper mutations
    //法一：设计定时器:
  //   setTimeout(() => {
